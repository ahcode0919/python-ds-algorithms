class TrieWithFlatDictionary:
    ...
    Unlike the array-backed and dictionary-backed tries, this implementation does not build an explicit
    tree of node objects at all. Instead, every prefix of every inserted word is stored directly as a key
    in a single flat dictionary, mapped to a boolean indicating whether that prefix is itself a complete
    word. This trades memory for simplicity: lookups are O(1) since accessing elements is a direct
    dictionary access, but space is O(M * N) because multiple values of a key must be tracked (plus their
    associated True/False values), since every prefix of every word is stored as its own entry rather than
    being shared through a tree structure.
    """
