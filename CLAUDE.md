...
- **`algorithm_theory/`** — canonical/fundamental algorithm approaches (e.g. binary/jump/linear search, bubble
  sort). Explanations for these live as **module-level docstrings in the `.py` files themselves**, not in a
  README — this directory intentionally deviates from the pattern below.
...
- **Topic/problem directories** (`arrays/`, `binary_search/`, `binary_tree/`, `circularly_linked_list/`,
  ...
  `singly_linked_lists/`, `stacks/`, `strings/`, `trie/`) — one function (or small set of related functions) per
  file solving a specific problem. Each of these directories has a `README.md` that duplicates the problem
  description and solution code inline as documentation (see any existing entry, e.g. `stacks/README.md`, as the
  template when adding a new problem/README section).
