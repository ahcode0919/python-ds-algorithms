...
This solution uses two pointers that each walk one list then switch to the head of the other list, so they cover
equal total distance and meet at the intersection (or both reach None if there is none). Time: O(N), Space: O(1).
"""
...
def get_intersection_node(
    ...
    """Walk both lists in lockstep, switching each pointer to the other list's head when it runs out."""
