def license_key_formatter(license_str: str, group_length: int) -> str:
    ...
    Given a license string which consists only of alphanumeric characters and dashes, the string is separated
    into `N+1` groups by `N` dashes.

    Given a `group_length`, reformat the string so that each group contains exactly that many characters, except
    for the first group, which may be shorter but must still contain at least one character. There must be a
    dash inserted between groups, and all lowercase letters should be converted to uppercase.
