...
def jump_search(array: [int], target: int) -> Optional[int]:
    ...
    search) by jumping ahead by fixed steps or skipping some elements in place of searching all elements.

    For example, suppose we have an array arr[] of size n and block (to be jumped) size `m`. Then we search
    at the indexes `arr[0]`, `arr[m]`, `arr[2m]…..arr[km]`, and so on. Once we find the interval
    (`arr[km] < x < arr[(k+1)m]`), we perform a linear search operation from the index `km` to find `x`.
    ...
    minimum when `m = √n`. Therefore, the best step size is `m = √n`. (Geeks For Geeks)
    """
    length = len(array)
